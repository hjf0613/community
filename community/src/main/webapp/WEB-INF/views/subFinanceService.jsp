		...
		<div class="body">
			<div class="row">
				<div class="col-md-2 booktype">
					...
					<c:forEach items="${listFinanceService}" var="item">
						
					  <li class="list-group-item"><a href="/listOneFinanceService/${item.serviceId}">${item.serviceName}</a></li>
					</c:forEach>
					...
					<c:forEach items="${rinfoList}" var="rinfo">
							<div class="one-book">
								<div class="row">
									<div class="col-md-3 book-image">
										<img src="<%=basePath%>assets/foreground/image/finance.jpg" alt="..." width="90px" height="110px"/>
									</div>
									<div class="col-md-9 book-content" style="padding-top:5px;">
										...
										<div class="row" style="padding-top:15px;">
											...
											<div class="col-md-7">
												...
												<button type="button" class="btn btn-success" style="margin-right:13px;"> <span class="glyphicon glyphicon-eye-open"></span> <a href="/oneFinanceRinfo/${rinfo.rinfoId}/${rinfo.service.serviceId}/${rinfo.league.leagueId}">查看详情</a></button>
							...
							<div class="pagination pagination-right" style="margin-left:720px;">  
						        <font size="2">共 ${page.totalPageCount} 页</font> <font size="2">第  
						            ${page.pageNow} 页</font> <a href="/initFinance?pageNow=1">首页</a>  
						        <c:choose>  
						            <c:when test="${page.pageNow - 1 > 0}">  
						                <a href="/initFinance?pageNow=${page.pageNow - 1}">上一页</a>  
						            </c:when>  
						            <c:when test="${page.pageNow - 1 <= 0}">  
						                <a href="/initFinance?pageNow=1">上一页</a>  
						            </c:when>  
						        </c:choose>  
						        <c:choose>  
						            <c:when test="${page.totalPageCount==0}">  
						                <a href="/initFinance?pageNow=${page.pageNow}">下一页</a>  
						            </c:when>  
						            <c:when test="${page.pageNow + 1 < page.totalPageCount}">  
						                <a href="/initFinance?pageNow=${page.pageNow + 1}">下一页</a>  
						            </c:when>  
						            <c:when test="${page.pageNow + 1 >= page.totalPageCount}">  
						                <a href="/initFinance?pageNow=${page.totalPageCount}">下一页</a>  
						            </c:when>  
						        </c:choose>  
						        <c:choose>  
						            <c:when test="${page.totalPageCount==0}">  
						                <a href="/initFinance?pageNow=${page.pageNow}">尾页</a>  
						            </c:when>  
						            <c:otherwise>  
						                <a href="/initFinance?pageNow=${page.totalPageCount}">尾页</a>  
						            </c:otherwise>  
						        </c:choose>  
						    </div>
